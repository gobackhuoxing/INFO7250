...
public class ProcessPicture {
		...
		for (AnnotateImageResponse res : responses) {
			...
			for (EntityAnnotation annotation : res.getLabelAnnotationsList()) {
				...
				for(Entry<FieldDescriptor, Object> entry : map.entrySet()){
					if(count%3==2 && entry!=null&&keywordFilter(entry.getValue().toString())) keyword.add(entry.getValue().toString());
					count++;
				}
			}
		}
	...
	//delete the too common keyword
	public boolean keywordFilter(String keyword){
		boolean pass =true;
		String[] deletelist = new String[]{"person","human","people","town","color","nature","road","dish","photo","presentation",
				"close","face","hair style","entertain","faciel","room","weather"};
